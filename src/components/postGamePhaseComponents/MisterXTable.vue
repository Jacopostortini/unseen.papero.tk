<template>
  <div class="mister-x-table__main-panel">
    <div class="mister-x-table__turn__wrapper" v-for="n in gameConfig.number_of_total_turns" :key="n">
      ...
      <div class="mister-x-table__turn-transport" :style="{
        backgroundColor: misterXMoves[n-1] ? colors[misterXMoves[n-1].transport] : 'none',
        border: misterXMoves[n-1] ? (misterXMoves[n-1].transport===3 ? '1px solid white' : 'none' ): 'none'
     }">
        <strong v-if="misterXMoves[n-1] ? misterXMoves[n-1].position : false"
                :style="{
          color: misterXMoves[n-1] ? (misterXMoves[n-1].transport===0 ? 'black' : 'white') : 'none'
                }">
